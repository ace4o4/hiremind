...
import {
  ...
  LayoutDashboard, MessageSquare, Briefcase, ArrowUpRight, StopCircle, RefreshCw, Upload, Eye
} from "lucide-react";
              ...
              {(["upload", "configure", "ready"] as const).map((s, i) => (
                <div key={s} className="flex flex-1 items-center gap-2">
                  <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold transition-all duration-500 ${
                    step === s ? "neu-convex text-blue-600" : i < ["upload", "configure", "ready"].indexOf(step) ? "neu-flat text-blue-500" : "neu-pressed text-slate-400"
                  }`}>
                    {i + 1}
                  </div>
  ...
  return (
    <div className="min-h-screen bg-deep-space">
      ...
      <header className="sticky top-0 z-30 flex h-20 items-center justify-between px-6 lg:px-12 bg-[#e0e5ec]/80 backdrop-blur-md">
        <div>
          <h1 className="font-display text-xl font-black text-slate-800">Welcome back, Alex 👋</h1>
        ...
        <div className="flex items-center gap-6">
           ...
           <div className="flex items-center gap-3 pl-6 border-l border-slate-300">
              <div className="flex flex-col items-end">
                 <span className="text-sm font-black text-slate-800 leading-tight">Alex Chen</span>
                 <span className="text-[10px] font-bold text-slate-500 uppercase">Free Plan</span>
              </div>
              <div className="w-10 h-10 rounded-full neu-convex flex items-center justify-center p-0.5">
                 <div className="w-full h-full rounded-full bg-slate-200 overflow-hidden shadow-inner">
                   <img src="https://images.unsplash.com/photo-1531427186611-ecfd6d936c79?q=80&w=150&auto=format&fit=crop" alt="Profile" className="w-full h-full object-cover" />
                 </div>
              </div>
           </div>
        </div>
